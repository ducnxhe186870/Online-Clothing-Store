 ...
 * @author ACER
 */
@WebServlet(name="HomeServlet", urlPatterns={"/view/Home"})
public class HomeServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        //Lay list blog
        List<Post> list_blog = podb.get6NewForHome();
        ProductImageDAO pidb = new ProductImageDAO();
        for (int i = 0; i < list_product.size(); i++) {
            list_img.add(pidb.getImageById(list_product.get(i).getProID()));
        }
        List<Stock> list_stock = sdb.getStockForShopAll(list_product);
